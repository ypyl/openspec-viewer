...
At viewport widths of 62em or more, the system SHALL provide a header visibility control for the review panel and a header visibility control for the file list sidebar. Each control SHALL toggle its panel between visible and hidden, SHALL indicate the current state of its panel, SHALL be hidden from the interface below 62em, and SHALL NOT affect the folder rail, which SHALL remain in the layout at all times. Hiding a panel SHALL remove it from the layout so the content pane re-expands to fill the freed space; showing it SHALL restore it in place. Neither panel SHALL overlap the artifact content while visible.

#### Scenario: Hiding the review panel widens the pane
- **WHEN** the viewport is at least 62em wide and the user activates the review panel's visibility control while the panel is visible
...
- **WHEN** the viewport is at least 62em wide, the review panel is hidden, and the user activates its visibility control to show it
...
- **WHEN** the viewport is at least 62em wide and the user activates the file list's visibility control
...
- **THEN** each header visibility control indicates whether its panel is currently visible or hidden
