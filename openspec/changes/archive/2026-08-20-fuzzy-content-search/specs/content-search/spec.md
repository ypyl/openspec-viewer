...
Lets the user find any OpenSpec document by its content: a fuzzy-search box in the app header returns snippet results from across all sections, and each result deep-links into the matching artifact with the matching text highlighted.
...
The system SHALL open the artifact belonging to a clicked result in its usual surface: an artifact inside a change (active or archived) opens the change view with that artifact's tab active, and a standalone spec or configuration file opens directly. After opening, the matching text SHALL be visually highlighted within the rendered artifact and the first match SHALL be scrolled into view.
...
- **THEN** the pane opens that change with the matching artifact's tab selected and the matched text highlighted and scrolled into view

#### Scenario: Clicking a spec result opens the spec
- **WHEN** the user clicks a result whose artifact is a standalone spec
- **THEN** the pane opens that spec with the matched text highlighted and scrolled into view

#### Scenario: Clicking a config result opens the config
- **WHEN** the user clicks a result whose artifact is a configuration file
- **THEN** the pane opens that file with the matched text highlighted and scrolled into view
...
- **THEN** the pane opens the archived change with the matching artifact's tab selected and the matched text highlighted and scrolled into view

### Requirement: Results track the current folder contents

The system SHALL base search results on the current state of the opened folder: artifacts that appear, change, or are removed by the live folder scan SHALL be reflected in search results without reloading the page. Search SHALL also work for folders loaded through the file-upload fallback.

#### Scenario: Newly added artifact becomes searchable
- **WHEN** a new artifact appears in the folder during live monitoring
...
- **WHEN** an artifact is removed from the folder during live monitoring
