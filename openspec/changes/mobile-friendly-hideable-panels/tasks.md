...
- [ ] 4.4 Commit everything (UI work and version bump together), push to `master`, wait for the GitHub Pages build (~1 min), and confirm the header badge reads v3.6.0 at https://ypyl.github.io/openspec-viewer/.
- [ ] 4.5 Confirm `screenshot.png` needs no re-shoot: at 1440×900 the desktop layout is visually identical to before this change (rail and sidebar remain pinned columns; the drawer and its toggle never render at that width).